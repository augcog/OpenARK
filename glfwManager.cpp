...
void Object::set_transform(const Eigen::Affine3d& t){ // Moon : Cause 3.a. const looks fine here.
    std::lock_guard<std::mutex> guard(displayLock_);
    pose = t;
}
...
void Object::rotate(const Eigen::Quaterniond& q){ // Moon : Cause 3.a. This change might be wrong. const + & or & alone should be added.
    std::lock_guard<std::mutex> guard(displayLock_);
    pose = q*pose;
}
