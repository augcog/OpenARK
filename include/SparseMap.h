...
class SparseMap {
 public:
  ...
  void getTrajectory(std::vector<Eigen::Matrix4d, const Eigen::aligned_allocator<Eigen::Matrix4d>>& trajOut){ // Moon: Cause 4 = Cause 2.a + Cause 3. const added.
  ...
  void getMappedTrajectory(std::vector<int>& frameIdOut, std::vector<Eigen::Matrix4d, const Eigen::aligned_allocator<Eigen::Matrix4d>>& trajOut){ // Moon: Cause 4 = Cause 2.a + Cause 3. const added.
    for(std::map<int, MapKeyFrame::Ptr>::iterator frame = frameMap_.begin(); 
