...
namespace ark
{
    ...
    void ark::CorrespondenceRansac<PointT>::getInliersWithTransform(CloudPtr src, CloudPtr tgt, const std::vector<int>& correspondences, int num_samples, float inlier_threshold, int num_iterations, int& num_inliers_out, std::vector<bool>& inliers_out, Eigen::Affine3d transform_out)
    {
    ...
    template<typename PointT>
    bool ark::CorrespondenceRansac<PointT>::checkThreshold(const PointT& p1, const PointT& p2, float inlier_threshold)
    {
    ...
    template<typename PointT>
    ark::CorrespondenceRansac<PointT>::CorrespondenceRansac()
    {
    }
}