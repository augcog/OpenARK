...
namespace ark {
    ...
    template <class DescType, class Feat> int OkvisSLAMSystem<DescType, Feat>::getActiveMapIndex() {
        return active_map_index;
    }

    template int OkvisSLAMSystem<BRISKDescType, BRISKDesc>::getActiveMapIndex();
    template int OkvisSLAMSystem<ORBDescType, ORBDesc>::getActiveMapIndex();

    template <class DescType, class Feat> bool OkvisSLAMSystem<DescType, Feat>::getMapTrajectory(int index, std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d>>& trajOut) {
        if (sparse_maps_.find(index) == sparse_maps_.end()) {
            return false;
        } else {
            sparse_maps_[index]->getTrajectory(trajOut);
            return true;
        }
    }

    template bool OkvisSLAMSystem<BRISKDescType, BRISKDesc>::getMapTrajectory(int index, std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d>>& trajOut);
    template bool OkvisSLAMSystem<ORBDescType, ORBDesc>::getMapTrajectory(int index, std::vector<Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d>>& trajOut);
